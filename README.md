...
#### Google Drive "구글로 로그인" 연결 설정 (배포 시 한 번)

배포된 ClassLog에서 선생님이 **버튼 하나로** Google Drive 동기화를 켜시려면 배포에 OAuth Client ID를 환경변수로 심어둡니다.

1. <https://console.cloud.google.com/>에서 프로젝트 생성
2. **API 및 서비스 → 사용자 인증 정보 → OAuth 클라이언트 ID(웹 애플리케이션)** 발급
3. 승인된 JavaScript 원본에 **운영 도메인**(예: `https://classlog-xxx.vercel.app`)과 **로컬 개발 도메인**(`http://localhost:3000`) 등록
4. **Google Drive API** 사용 설정 + OAuth 동의 화면에서 **선생님 본인 이메일**을 테스트 사용자로 추가
5. Vercel **Settings → Environment Variables** 에서 다음 추가 후 **Redeploy**
   ```
   NEXT_PUBLIC_GOOGLE_CLIENT_ID = xxxxxxxxxxxx.apps.googleusercontent.com
   ```

이렇게 해두면 선생님 화면에서는 **설정 → Google Drive 동기화 → "구글로 로그인"** 한 번이면 끝입니다. 셋업 안내·Client ID 입력 칸은 자동으로 숨겨집니다.

> 이 env 변수를 비워두면(설정 안 하면) 선생님이 직접 Client ID를 붙여넣는 모드가 활성화됩니다 — 자체 호스팅이나 한 분이 직접 관리하실 때 사용합니다.
